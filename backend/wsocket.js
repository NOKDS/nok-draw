...
module.exports = function setupSocketServer(io) {
  ...
  io.on("connection", (socket) => {
    socket.on("joinGame", (data) => {
      ...
      if (!roomCode) {
        // If no room code provided, add to queue
        queue.push(socket);
      } else {
        // If room code provided, join existing room
      ...
      if (queue.length === 4) {
        ...
        setTimeout(async () => {
          ...
          if (roomUsers && roomName in roomUsers) {
            const canvasStrings = Object.values(roomUsers[roomName]).map(
              (value) =>
                Object.fromEntries([
                  ["userId", value.userId],
                  ["canvasString", value.canvasString],
                ])
            );
            // console.log("CANVAS STRINGS", canvasStrings)
            try {
              const response = await axios.post(
                "http://127.0.0.1:5000/predictMany",
                {
              ...
              const winners = [];
              const falseProbability = [
                0.18275894224643707, 0.08449413627386093, 0.08340463787317276,
                0.0820060670375824,
              ];
              for (let id in userGuesses) {
                ...
                for (let i = 0; i < userGuesses[id].length; i++) {
                  ...
                  if (
                    array[0] == topic &&
                    !falseProbability.includes(array[1])
                  ) {
              ...
              for (let i = 0; i < winners.length; i++) {
                ...
                userStats[winners[i][0]] = {
                  place: place,
                  top4Guesses: userGuesses[winners[i][0]].map(
                    (value) => value[0]
                  ),
                };
              }
                ...
                if (socket.email) {
                  try {
                    ...
                    await Game.create({
                      ...
                      category: topic,
                      top3Predications: JSON.stringify(
                        userStats[socket.userId].top4Guesses
                      ),
                      image:
                        roomUsers[roomName][socket.userId].getCanvasString(),
                      UserId: user.id,
                    });
    ...
    socket.on("canvasData", async (data) => {
      ...
      if (
        roomUsers &&
        data.roomName in roomUsers &&
        data.userId in roomUsers[data.roomName]
      ) {
      ...
      if (roomName && rooms[roomName]) {
        ...
        if (socketIndex !== -1) {
          ...
          if (rooms[roomName].length === 0) {
            ...
            const allUsers = rooms[roomName]
              .filter((player) => player.connected)
              .map((player) => player.userId);
            // console.log('all users', allUsers)
            for (let sock of rooms[roomName]) sock.emit("users", allUsers);
