...
const GameRoom = ( ) => {
    ...
    const [userCanvases, setUserCanvases] = useState<Map<string, UserCanvas>>(new Map());
    ...
    useEffect(() => {
      ...
      const userCanvas: UserCanvas | null | undefined = userCanvases.get(userId) ? userCanvases.get(userId) : null;
    ...
    useEffect(() => {
      ...
      setInterval(() => {
        ...
        socketRef.current.emit('canvasData', { base64Img, userId, roomName: roomnameRef.current});
    ...
    return (
        <div>
                    ...
                    <span key={userId}>
                        <canvas
                            style={{
                                ...
                                cursor: isErasing ? `url('${eraserCursor}'), auto` : `url('${pencilCursor}'), auto`
                            }}
            ...
            <button onClick={() => {setIsErasing(erasing => !erasing)}}>{isErasing ? 'Draw' : 'Erase'}</button>
        </div>
    )
}
